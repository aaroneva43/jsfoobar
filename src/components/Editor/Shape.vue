  ...
  methods: {
    ...
    handleMouseDownOnShape(e) {
      ...
      const move = (moveEvent) => {
        ...
        const curX = moveEvent.clientX;
        const curY = moveEvent.clientY;
        pos.top = ((curY - startY + startTop*4/3)*3) / 4;
        pos.left = ((curX - startX + startLeft*4/3)*3) / 4;
        ...
        const rect = this.$el.getBoundingClientRect();
        if (
          (pos.left * 4) / 3 + rect.width >
            ((self.canvasStyleData.width * 4) / 3) *
              self.canvasStyleData.scale *
              0.01 ||
          (pos.top * 4) / 3 + rect.height >
            ((self.canvasStyleData.height * 4) / 3) *
              self.canvasStyleData.scale *
              0.01
        ) {
          return;
    ...
    handlekeydown(e) {
      ...
      e.stopPropagation();
      e.preventDefault();
      var l = e.currentTarget.offsetLeft;
      var t = e.currentTarget.offsetTop;
      const pos = { ...this.defaultStyle };
      if (e.keyCode === 37) {
        l -= 4 / 3;
        pos.left -= 1;
      ...
      e.currentTarget.style.left = l + "px";
    ...
    handleMouseDownOnPoint(point, e) {
      ...
      const move = (moveEvent) => {
        ...
        needSave = true;
        const curPositon = {
          x: (moveEvent.clientX - editorRectInfo.left)/4*3,
          y: (moveEvent.clientY - editorRectInfo.top)/4*3,
        };
